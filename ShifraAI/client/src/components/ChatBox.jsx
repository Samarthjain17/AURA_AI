...
const ChatBox = ({ user }) => {  // 👈 user prop yahan receive kiya
  const [prompt, setPrompt] = useState("");
  const [messages, setMessages] = useState([]);
  const [isLoading, setIsLoading] = useState(false);

  // Jab page load ho, purani history fetch karo
  useEffect(() => {
    const fetchHistory = async () => {
      if (!user?.email) return; // Jab tak email na aaye, ruko
      ...
      try {
        ...
        if (response.data.messages.length > 0) {
          ...
          // Agar history khali hai toh Welcome message dikhao
  ...
  const handleSend = async (e) => {
    ...
    try {
      // Ab request me user ka email bhi bhej rahe hain
      ...
      setMessages((prev) => [
        ...prev, 
        { role: "ai", text: "Oops! Connection error. 🔴" }
      ]);
    } finally {
      setIsLoading(false); 
    }

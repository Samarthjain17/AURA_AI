...
const ChatBox = ({ user, currentChatId, isTemporary }) => {
    ...
    // ChatBox.jsx ke andar ye useEffect replace kar do:
    useEffect(() => {
        const fetchHistory = async () => {
            // LocalStorage se ID check karo agar prop se nahi mil rahi
            const activeId = currentChatId || localStorage.getItem('activeChatId');

            if (!user?.email || !activeId) return;

            // Agar humne LocalStorage se ID li hai, toh use state me bhi set kar do
            if (!currentChatId && activeId) {
                // (Optional: Agar tumhare paas setCurrentChatId prop hai toh use call karo)
            }
            ...
            try {
                ...
                setMessages(response.data.messages.length > 0 ? response.data.messages : [{ role: "ai", text: "Hello! How can I assist you today? ✨" }]);
            } catch (error) {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto flex flex-col h-[90vh] bg-gradient-to-b from-white/[0.03] to-transparent backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-[0_8px_32px_0_rgba(0,0,0,0.4)] relative">
                ...
                <div className="flex flex-col gap-8">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} group animate-fade-in-up`}>
                            ...
                            <div className={`max-w-[85%] md:max-w-[75%] p-5 rounded-3xl shadow-lg relative ${msg.role === 'user' ? 'bg-gradient-to-br from-purple-600 to-indigo-600 text-white rounded-br-sm' : 'bg-[#131825]/90 border border-white/5 text-gray-200 rounded-bl-sm'}`}>
                                {msg.role === 'user' ? (
            ...
            <div className="px-4 pb-4 sm:px-6 sm:pb-6 bg-transparent relative">
                {showAttachMenu && (
                    <div ref={menuRef} className="absolute bottom-[85px] left-8 bg-[#1A2031] border border-gray-700 rounded-2xl shadow-2xl p-2 flex flex-col gap-1 z-50 animate-fade-in-up w-56">
                ...
                <form onSubmit={handleSend} className="flex flex-col bg-[#131825]/90 rounded-[32px] border border-gray-700/60 shadow-[0_4px_20px_rgba(0,0,0,0.3)] focus-within:border-purple-500/50 focus-within:ring-1 focus-within:ring-purple-500/30 transition-all overflow-hidden relative">
                    {imagePreview && (
